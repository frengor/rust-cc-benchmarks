...
enum TreeNodeWithParent {
    Root {
        left: Rc<TreeNodeWithParent>,
        right: Rc<TreeNodeWithParent>,
    },
    Nested {
...
impl TreeNodeWithParent {
    fn new(depth: usize) -> Rc<Self> {
        if depth == 0 {
            return Rc::new(Self::End);
        }

        Rc::new_cyclic(|weak| Self::Root {
            left: Self::new_nested(depth - 1, weak),
            right: Self::new_nested(depth - 1, weak),
        })
    }

    fn new_nested(depth: usize, parent: &Weak<Self>) -> Rc<Self> {
        if depth == 0 {
            return Rc::new(Self::End);
        }

        Rc::new_cyclic(|weak| Self::Nested {
            parent: parent.clone(),
            left: Self::new_nested(depth - 1, weak),
            right: Self::new_nested(depth - 1, weak),
        })
