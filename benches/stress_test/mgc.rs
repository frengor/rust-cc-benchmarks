...
struct DirectedGraphNode {
    _label: String,
    edges: Vec<Gc<Mutex<DirectedGraphNode>>>,
}

impl Trace for DirectedGraphNode {
    fn trace(&self) {
        self.edges.iter().for_each(|elem| elem.trace());
    }
}
...
fn stress_test(rng: &mut StdRng) -> Vec<usize> {
    let mut res = Vec::new();
    {
        ...
        for i in 0..=NODE_COUNT {
            nodes.push(Gc::new(Mutex::new(DirectedGraphNode {
        ...
        for _ in 0..=EDGE_COUNT {
            ...
            a.lock().edges.push(Gc::clone(b));
        }

        for i in 0..NODE_COUNT {
            if i % SHRINK_DIV == 0 {
                nodes.truncate(NODE_COUNT - i);
                nodes.iter().for_each(|elem| elem.trace());
                sweep();
                res.push(nodes.len());
            }
        }
...
pub fn benchmark_stress_test(c: &mut BenchmarkGroup<impl Measurement>, rng: &mut StdRng) {
    c.bench_function("mgc", |b| b.iter_with_large_drop(|| stress_test(black_box(rng))));
}
